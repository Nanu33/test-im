...
class Preview_Pool_Details extends Component {
  constructor(props) {
    super(props);
    this.state = {
      selectedRecords: [],
  ...
  sortLoanDetails = () => {
    ...
    // Sort loandetails based on the order of Loan ID in notificationlist
  ...
  handleClicks = () => {
    console.log("hello we have clicked the button");
    window.location.assign("/admin/uw_preview_pools");
    // this.props.history.push({
    //     pathname: "/admin/uw_preview_pools",
    // });
  };
  PoolDetails1 = async () => {
    try {
      ...
      if (APIResponse.status === 200) {
        ...
        const staticColumn = [
          ...
          },
          {
            ...
            //   <ClearOutlinedIcon
            //     onClick={() =>
            //       this.onOpenStatusUpdate(tableMeta.rowData[0])
            //     }
            //   />
            // </button>
            //         <button className="search-mui-icons">
  ...
  //                                 return (
  //                                     <React.Fragment>
  //                                         {this.state.poolName === "New Silver" ? tableMeta.columnIndex === 6 ? (
  //                                             <NumberComp className="text-center" value={value}></NumberComp>
  //                                         ) : tableMeta.columnIndex === 7 ? (
  //                                             <div className="text-center">{(value * 100).toFixed(3) + '%'}</div>
  //                                         ) :
  //                                             (<div className="text-center">{value}</div>) : <div className="text-center">{value}</div>}
  //                                     </React.Fragment>
  //                                 );
  //                             },
  //                         },
  ...
  //                 let checkerBoolen = arr => arr.every(v => v['Contract Data'] === 'Yes');
  ...
  //                 if (checkLoanContratData === true && checkPoolAlert === "False" && this.state.userrole === "Issuer") {
  //                     finalCheck = true;
  //                 } else if (checkLoanContratData === false && checkPoolAlert === "False" && this.state.userrole === "Issuer") {
  //                     finalCheck = true;
  //                     this.setState({
  //                         noAlertStatus: "No"
  //                     })
  ...
  //                 ]
  //                 const allColumns = [...staticColumn, ...display]
  //                 this.setState({ columns: allColumns, isSubmitVerifiedBtn: finalCheck, open8: finalCheck });
  //                 this.SetUpPoolRoles()
  //             } else {
  //                 this.setState({
  //                     tableData1: [],
...
export default withSnackbar(Preview_Pool_Details);
