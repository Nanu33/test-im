...
class InvestorPreviewPools extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  previewInvestorPool = async () => {
    ...
    data.token = this.state.token;
    try {
      const APIResponse = await previewInvestorPool(data);
      if (APIResponse.status === 200) {
        this.setState(
          {
            getLoansLoader: false,
          ...
          () => {
            this.sortPoolDetails();
            // this.sortPoolDetails(APIResponse.data?.pooldetails, APIResponse.data?.notificationlist);
            if (
              APIResponse.data.pooldetails &&
              APIResponse.data.pooldetails.length > 0 &&
  ...
  //     sessionStorage.setItem("verificationDropdown", JSON.stringify(this.state.VerificationDropdown));
  ...
  //     sessionStorage.setItem("underwriterDropdown", JSON.stringify(this.state.UnderwriterDropdown))
...
export default withSnackbar(InvestorPreviewPools);
